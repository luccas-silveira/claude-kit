...
class Registro(unittest.TestCase):
    ...
    def test_impeccable_com_guarda(self):
        local = os.path.join(RAIZ_CLAUDE, 'settings.local.json')
        if not os.path.exists(local):
            self.skipTest('a cópia do backup não leva o settings.local.json')
        with open(local) as f:
            h = json.load(f)['hooks']
        impeccable = [x['command'] for g in h.get('Stop', []) for x in g['hooks'] if 'impeccable' in x['command']]
        self.assertTrue(impeccable)
        for c in impeccable:
            self.assertRegex(c, GUARDA)
