...
description: Escreve, audita ou refatora o system prompt (instruções de sistema) de um agente, assistente, bot ou chatbot de IA, aplicando os padrões de engenharia de prompt que CONVERGEM entre os líderes do setor (Anthropic, OpenAI, Google, xAI, Meta, Moonshot, Mistral, MiniMax, Perplexity, Hume), extraídos por análise de frequência cruzada de 42 system prompts reais. Use sempre que o usuário for definir o comportamento de um LLM via instruções de sistema: montar do zero o prompt de um agente de vendas, atendimento, RH, jurídico, suporte ou de qualquer domínio/canal (WhatsApp, Instagram, site, voz); deixar um prompt amador mais profissional, consistente ou robusto; consertar um agente que alucina ou inventa dados; ou auditar/revisar um prompt existente e diagnosticar o que falta. Cobre também "boas práticas de prompt" e "prompt engineering" aplicados a instruções de sistema. NÃO use para: prompts de imagem (Midjourney/DALL-E), prompts pontuais pra colar numa conversa de ChatGPT, redação de e-mails/legendas/posts, nem dúvidas de API, fine-tuning ou RAG. Para o racional técnico de cada padrão, leia guia-prompt-engineering.md.
---
